...
const shouldType = ref(false);

const typeNextChar = (index) => {
    // Si se cerró el regalo, detener
    if (!shouldType.value) {
    ...
    if (index < fullMessage.length) {
        displayedText.value = fullMessage.substring(0, index + 1);
        setTimeout(() => typeNextChar(index + 1), 30);
    } else {
        isTyping.value = false;
    }
};
